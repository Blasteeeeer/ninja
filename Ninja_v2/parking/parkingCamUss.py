...
def initializeTrackbars(intialTracbarVals, wT=480, hT=240):
    cv2.namedWindow("TopView_park")
    cv2.createTrackbar("Width Top", "TopView_park", intialTracbarVals[0], wT // 2, nothing)
    cv2.createTrackbar("Height Top", "TopView_park", intialTracbarVals[1], hT, nothing)
    cv2.createTrackbar("Width Bottom", "TopView_park", intialTracbarVals[2], wT // 2, nothing)
    cv2.createTrackbar("Height Bottom", "TopView_park", intialTracbarVals[3], hT, nothing)
    
    cv2.namedWindow("ColorMask")
    cv2.createTrackbar("lower_h", "ColorMask", 0, 255, nothing)
    cv2.createTrackbar("lower_s", "ColorMask", 75, 255, nothing)
    cv2.createTrackbar("lower_v", "ColorMask", 90, 255, nothing)
    cv2.createTrackbar("upper_h", "ColorMask", 23, 255, nothing)
...
def thresholding(img):
    ...
    if color_area_num > 0: 
        for i in contours:    # Traverse all contours
            ...
            print("area",w* h)
            if (w*h)>1800: # Because the picture is reduced to a quarter of the original size, if you want to draw a rectangle on the original picture to circle the target, you have to multiply x, y, w, h by 4.
                is_Detect = True

    return morph_mask, is_Detect

def valTrackbars(wT=480, hT=240):
    widthTop = cv2.getTrackbarPos("Width Top", "TopView_park")
    heightTop = cv2.getTrackbarPos("Height Top", "TopView_park")
    widthBottom = cv2.getTrackbarPos("Width Bottom", "TopView_park")
    heightBottom = cv2.getTrackbarPos("Height Bottom", "TopView_park")
...
def ParkDecision(parkDir):
    ...
    #Maximum n`umber of Parking spaces
    ParkingspaceCount = 1
...
#     bot_drive.reverse(2)
#     time.sleep(10)
    #while (Front_distance >= 8):
    print("Forward")
    print("Front distance for first forward",Front_distance)
    bot_drive.forward(5)
    time.sleep(3.5)
    bot_drive.forward(0)
    time.sleep(1)
...
def main():
    ...
    #parking
    intialTracbarVals = [0, 180, 0, 240]
    initializeTrackbars(intialTracbarVals)
    ussParkSpaceAvailable = False
    
    #lane
    lane_intialTrackBarVals = [35, 227, 0, 240]
#     lane_intialTrackBarVals = [0, 195, 0, 240]
    utlis.initializeTrackbars(lane_intialTrackBarVals)
...
#         gray = cv2.cvtColor(BridView, cv2.COLOR_BGR2GRAY)
        ...
        # lane
#         curve, curveList, imgResult, imgWarp = lane_detection.getLaneCurve(frame,curveList, display=1)
#         
#             
#         cv2.imshow("imgWarp", imgWarp)
#         print("Curve value = ", curve)
#         
#         steer_angle = lane_detection.Find_steer(curve)
#         print("steer_angle", steer_angle)
#         servo.set_SteeringAngle(steer_angle)


        ### steering based on USS
        left_dist = uss.getDistance('left')
        right_dist = uss.getDistance('right')
        
        front_dist = uss.getDistance('front')
        ...
        print("print left dist", left_dist)
        print("print right dist", right_dist)
        
        if (left_dist + right_dist) <= 105 and (left_dist + right_dist) >= 80:
            if left_dist > 45 and left_dist < 55:
#                 steer right with more angle
                servo.set_SteeringAngle(10)
            elif left_dist > 35 and left_dist <45 :
                # steer right with less angle
                servo.set_SteeringAngle(15)
                
            if right_dist > 45 and right_dist < 55:
#                 steer left with more angle
                servo.set_SteeringAngle(-10)
            elif right_dist > 35 and right_dist <45:
                # steer left with less angle
                servo.set_SteeringAngle(-15)

        elif left_dist + right_dist > 105:
...
#             bot_drive.forward(0)
        ...
        if(is_Detect == True):
            ...
            if (parkingSide < -2):
                ...
                dist = bot_drive.get_veh_distance()
                init_dist = 9 # Distance to be moved once P sign detected
...
#                     bot_drive.forward(0)
                ...
                dist = bot_drive.get_veh_distance()
                init_dist = 11 # Distance to be moved once P sign detected
